...
SpiCore::SpiCore(uint32_t core_base_addr) {
    ...
    set_freq(100000); // set default frequency to 100kHz
    set_mode(0, 0);
    write_ss_n(0xFFFFFFFF); // deassert all SS lines
}
SpiCore::~SpiCore()  {}
...
void SpiCore::write_ss_n(int bit_val, int bit_pos) {
    bit_write(ss_n_data, bit_pos, bit_val);
    io_write(base_addr, SS_REG, ss_n_data);
}
